...
class internshala:
    ...
    def get_internship_info(self, url):
        ...
        if self.is_int_or_job in ['internship', 'internships']:
        ...
        if skills_loc.is_visible() and round_tabs_count >= 2:
            ...
            self.skills = "`Company doesn't mention particular skills. Assume skills according to Internship Description.`"
        ...
        # Handle "Apply now" and Cover Letter Not Found case
        try:
            apply_button = self.page.get_by_role("button", name="Apply now")
            apply_button.click()
            # Wait for the cover letter section, but don't stop if it's not found
            self.page.wait_for_selector('//*[@id="cover_letter_holder"]/div[1]', state='visible', timeout=15000)
        except TimeoutError:
            print("[bold yellow]Cover letter section not found. Proceeding with next steps...[/]")
    ...
    def fill_app_form(self, GPT, success, failed, validate_assignment_question):
        ...
        for x in range(count):
            ...
            # Check if question is of checkbox type (i.e., Yes/No style question)
            checkbox_question_selector = assignments.nth(x).locator('input[type="checkbox"]')
            if checkbox_question_selector.is_visible():
                self.page.evaluate("document.querySelector('input[type=\"checkbox\"]')[0].click()")
                print("[green]Checkbox question selected: Yes[/]\n")
